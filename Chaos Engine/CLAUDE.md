...
- `src/app/shared/` holds UI shared by both screens: `MapBoard` (atlas plus an SVG overlay; `dmView` shows hidden events; `zoomable`, on for the table, adds wheel or pinch zoom up to 5x about the pointer, drag to pan, and +, - and Whole map buttons. Markers grow by the square root of the zoom, and the click that ends a pan selects nothing), `MoraleMarks`, and `format`.
