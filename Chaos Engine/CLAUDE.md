...
- **Mother Dark Temples** (17 total, 5 active at the start): nearby corruption can't rise above 2, and Anomander's resources get +3 Power in range. The DM sets the radius globally: **Reach of every temple** in a temple's inspector, stored as `events.auraRadius` (null keeps `events-config.json`'s); always read it through `auraRadiusOf`. On the map a temple is a columned silhouette (`templeGlyph` in `shared/map-board.ts`), bright when active and faint when dormant. Activating a temple has a difficulty like a portal's.
...
- `src/app/data/` holds the Angular services:
  ...
  - `PortraitStore` keeps uploaded portraits (320 px JPEGs, cropped by `shared/portrait.ts`) in IndexedDB, **apart from `GameState`**, so autosaves and snapshots stay small. Both windows read it, and a `BroadcastChannel` tells the other window to reload. It uses the uploaded portrait first, then the seed's optional `image` path (under `public/`), then the monogram. Exported save files carry the portraits; autosaves and snapshots don't.
- `src/app/dm/` is the DM screen (`/dm`): top bar, resource ledger, map, event and resource inspectors, the castle panel, and the reckoning (review the rolls, commit, turn report). The DM brings new allies into play with **Add a hero or group** (`dm/new-resource-form.ts`, over `createResource` in `engine/dm-edits.ts`, which generates a readable unique id) and takes them out with **Remove from the game** (`removeResource`). Both are undoable.
...
- `src/app/shared/` holds UI shared by both screens: `MapBoard` (atlas plus an SVG overlay; `dmView` shows hidden events; `zoomable`, on for the table, adds wheel or pinch zoom up to 5x about the pointer, drag to pan, and +, - and Whole map buttons. Markers grow by the square root of the zoom, and the click that ends a pan selects nothing). `iconSizeKey` adds an Icons control (50% to 200% in tenths, the % button resets) that scales the event markers on that screen; it is a display preference kept in that browser's localStorage (`chaos-engine.icon-size.table` / `.dm`), never in `GameState`, `MoraleMarks`, and `format`.
- **UI tokens** are in `src/styles.scss`. The map is shown as drawn, with no dimming. Around it, the look is a war table at night: slate and bone, with colour only where it means something (brass for actions and gold; Chaos portals as embers, `--ce-chaos-1`..`5`, amber to red with corruption; violet for Mother Dark; oxblood for loss; verdigris for victory; rime for cold light and locations). Type is Alegreya and Alegreya Sans, bundled offline via `@fontsource`. Don't use all-caps labels. Numbers drawn in SVG must set lining figures (`font-variant-numeric: lining-nums`), because Alegreya's default old-style digits never sit centred.
...
- `commanders.json` lists the six player characters who can be chosen as commander. All are level 17. DM-controlled characters such as Johanna and Anuna are deliberately left out.
  ...
  - The DM rewrites any commander's sway in the app (**Edit sway** in the command picker, `dm/sway-editor.ts`): add, change (−2, −1, +1, +2) or remove, each with a reason, or restore the archive's. The edits live in `GameState.commanderInfluence` and replace the seed for that commander only (`influenceOf` in `engine/commanders.ts`). They count from the commander's next command; `GameState.commandSway` records what was applied this turn, and the table banner shows that.
...
- `events-config.json` holds every tunable number for events. `map.json` defines the maps, the active map, land detection, no-spawn zones and regions. A map may list `styles`: other drawings with the same framing (so every position holds). The DM picks one from **Map** on the DM map; it is `GameState.mapStyle`, both screens draw it through `shownMap`, and land detection always reads the map's own `asset`. `temples.json` holds the 17 Mother Dark temples (5 active). All are placeholders except Jhag Odhan.
